...
void logPumpDataToSD(time_t timestamp, float solutionTemp, float substrateTemp, float atmosTemp, float atmosHumidity, float substrateMoisturePercent, float lightIntensity)
{
  ...
  if (dataFile)
  {
    ...
    // CSV format
    char data_str[100];
    snprintf(data_str, sizeof(data_str), "%s,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f",
             timestamp_str, solutionTemp, substrateTemp, atmosTemp, atmosHumidity, substrateMoisturePercent, lightIntensity);
